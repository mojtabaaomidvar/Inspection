...
import { LayoutDashboard, Users, FileText, UserCheck, ClipboardCheck, Receipt, BarChart3, Settings, ChevronLeft, ChevronRight, ShieldCheck } from "lucide-react";
...
export function Sidebar({ active, onSelect, isExpanded = true, onToggle }: SidebarProps) {
  const handleToggle = () => {
    onToggle?.(!isExpanded);
  };
  ...
  return (
    <aside 
      className={cn(
        "relative flex shrink-0 flex-col border-r border-slate-800/60 bg-slate-950 text-slate-300 transition-all duration-300 ease-in-out h-screen",
        isExpanded ? "w-64" : "w-20"
      )}
    >
      {/* 🔑 Header با لوگو و دکمه Toggle */}
      <div className={cn(
        "flex items-center px-5 py-5 border-b border-slate-800/70",
        isExpanded ? "gap-3" : "justify-center"
      )}>
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 via-violet-500 to-fuchsia-500 shadow-lg shadow-indigo-900/50">
          <ShieldCheck className="h-5 w-5 text-white" />
        </div>
        
        {isExpanded && (
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold text-white truncate">ICS</div>
        ...
        <button
          ...
          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-800/60 hover:bg-slate-800 text-slate-400 hover:text-white transition-colors"
      ...
      <nav className="flex-1 space-y-0.5 px-3 overflow-y-auto">
        <div className={cn(
          "px-2 pb-2 pt-3 text-[10px] font-semibold uppercase tracking-wider text-slate-500",
